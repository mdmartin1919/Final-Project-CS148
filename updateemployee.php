...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
    ...
    if (isset($_POST["btnSubmit"])) {
        ...
        $results1 = $thisDatabase->select("SELECT pmkEmployeeNum FROM tblEmployee where pmkEmployeeNum like '%$employee%'");
        $employeeList = $results1[0]["pmkEmployeeNum"];
        
        $results1 = $thisDatabase->select("SELECT fldStatus FROM tblEmployee where pmkEmployeeNum = $employee");
        $adminList = $results1[0]["fldStatus"];
        
        if ($adminList == 'Admin'){
            $errorMsg[] = "You can not change an admin account.";
            $employeeERROR = true;
        } 
        ...
        if ($name == "" & $address == "" & $store == "" & $status == "" & $sex == "" & $pay == "" & $day == "" & $month == "" & $year == ""){
        ...
        if (!$errorMsg) {
            ...
            $results10 = $thisDatabase->select("SELECT fldName FROM tblEmployee where pmkEmployeeNum = $employee");
            $nameList = $results10[0]["fldName"];
            $results16 = $thisDatabase->select("SELECT fldHomeAddress FROM tblEmployee where pmkEmployeeNum = $employee");
            $addressList = $results16[0]["fldHomeAddress"];
            $results11 = $thisDatabase->select("SELECT fldStore FROM tblEmployee where pmkEmployeeNum = $employee");
            $storeList = $results11[0]["fldStore"];
            $results12 = $thisDatabase->select("SELECT fldStatus FROM tblEmployee where pmkEmployeeNum = $employee");
            $statusList = $results12[0]["fldStatus"];
            $results13 = $thisDatabase->select("SELECT fldSex FROM tblEmployee where pmkEmployeeNum = $employee");
            $sexList = $results13[0]["fldSex"];
            $results14 = $thisDatabase->select("SELECT fldDateHigher FROM tblEmployee where pmkEmployeeNum = $employee");
            $dateList = $results14[0]["fldDateHigher"];
            $results15 = $thisDatabase->select("SELECT fldPay FROM tblEmployee where pmkEmployeeNum = $employee");
            $payList = $results15[0]["fldPay"];
            ...
            try {
                ...
                $query = "update tblEmployee set fldName = ?, fldHomeAddress = ?, fldStore = ?,fldStatus = ?, fldSex = ?, fldDateHigher = ?, fldPay = ? where pmkEmployeeNum = ?";
                $data = array($name,$address,$store,$status,$sex,$datehighered,$pay,$employee);
...
//
                ...
                $messageA = '<h2>Your employee was updated!</h2>';
        ...
        ?>
            <form action="<?php print $phpSelf; ?>"
                ...
                <fieldset class="intro">
                    <legend>Complete the information to update an employee</legend>

                    <fieldset class="contact">
                        <label for="txtemployee" class="required">Employee Number
                            <input type="text" id="txtemployee" name="txtemployee" 
                                   value=""
                                   tabindex="100" maxlength="40" placeholder="enter an employee #" 
