...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
    ...
    if (isset($_POST["btnSubmit"])) {
        ...
        $results1 = $thisDatabase->select("SELECT pmkEmail FROM tblContact where pmkEmail like '$number'");
        $employeeList = $results1[0]["pmkEmail"];

        if ($number == "") {
            $errorMsg[] = "Please enter a correct Email";
            $numberERROR = true;
        } elseif (!verifyEmail($number)) {
            $errorMsg[] = "That email appears to be incorrect.";
            $numberERROR = true;
        ...
        if (!$errorMsg) {
            ...
            try {
                ...
                $query = "Delete from tblContact where pmkEmail like '$number'";
                $data = array($number);
...
//
                ...
                $messageA = '<h2>Your Contact info for: ' . $number . ' was removed!</h2>';
...
// display any error messages before we print out the form
            ...
            <form action="<?php print $phpSelf; ?>"
                ...
                <fieldset class="intro">
                    <legend>Enter the Email of the contact you want to delete</legend>

                    <label for="txtnumber" class="required">Contact's Email
                            <input type="text" id="txtnumber" name="txtnumber" 
                                   value=""
                                   tabindex="100" maxlength="40" placeholder="Enter the Email" 
